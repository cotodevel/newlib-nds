...
#ifndef PROTO_RSYNC_MISC
#define PROTO_RSYNC_MISC
...
uint32 get_checksum1(char *buf1, sint32 len);
void get_checksum2(char *buf, sint32 len, char *sum);
void file_checksum(const char *fname, const STRUCT_STAT *st_p, char *sum);
void sum_init(int csum_type, int seed);
void sum_update(const char *p, sint32 len);
...
struct map_struct *map_file(int fd, OFF_T len, sint32 read_size, sint32 blk_size);
char *map_ptr(struct map_struct *map, OFF_T offset, sint32 len);
...
void itemize(const char *fnamecmp, struct file_struct *file, int ndx, int statret,
	     stat_x *sxp, sint32 iflags, uchar fnamecmp_type,
	     const char *xname);
...
sint32 read_int(int f);
sint32 read_varint(int f);
...
void write_int(int f, sint32 x);
void write_varint(int f, sint32 x);
...
void write_ndx(int f, sint32 ndx);
sint32 read_ndx(int f);
...
void send_token(int f, sint32 token, struct map_struct *buf, OFF_T offset,
		sint32 n, sint32 toklen);
sint32 recv_token(int f, char **data);
void see_token(char *data, sint32 toklen);
